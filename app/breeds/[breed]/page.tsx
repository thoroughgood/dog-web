...
export default function Page() {
  ...
  return (
    <div className={`min-h-screen px-4 sm:px-8 md:px-16 lg:px-32 bg-blue-50 py-8 ${redHat.className}`}>
            ...
            {breed.name === "oodles" ? (
              <>
                ...
                <div className="flex flex-col text-center xl:text-left space-y-2">
                  {["Cavoodle", "Groodle", "Labradoodle", "Poodle", "Spoodle"].map((b) => (
                    <Link key={b} className="hover:text-blue-500 flex w-fit self-center" href={`./${b.toLowerCase()}`}>{b}</Link>
                  ))}
                </div>
                <div className={`${fredoka.className} text-red-500 text-center xl:text-left mt-4 flex gap-4 flex-row justify-center items-center`}>
                  <div className="">If you are interested in other breeds not listed here, email or call us!</div>
              ...
              <>
                <div><strong>Description:</strong> {breed.description}</div>
                <div><strong>Temperament:</strong> {breed.temperament}</div>
                <div><strong>Lifestyle:</strong> {breed.lifestyle}</div>
              </>
            )}
          </div>
        </div>
